...
part1 = '''    def __init__(self):
    ...
    def create_driver_content(self):
        ...
        self.tree.bind("<Control-a>", lambda e: [self.tree.selection_add(item) for item in getattr(self.tree, 'get_children', lambda: [])()])
    ...
    def create_restore_content(self):
        ...
        s2 = ttk.LabelFrame(self.content_frame, text="Helyreállítás: Driverek", padding=10)
        s2.pack(fill=tk.X, pady=10)
        ttk.Button(s2, text="Lementett Driverek Visszaállítása", command=self.restore_drivers).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(s3, text="Alap Driverek Kinyerése (install.wim)", command=self.extract_wim_drivers).pack(side=tk.LEFT, padx=5)

'''

part2 = '''    def check_wu_status(self):
        if self.target_os_path and self.target_os_path != self.sys_drive:
            self.wu_status_lbl.config(text=f"Állapot: OFFLINE Célrendszernél ({self.target_os_path}) nem lekérdezhető az állapot.", foreground="#B58900")
            return
        try:
            ...
            self.wu_status_lbl.config(text="Állapot: BEKAPCSOLVA (Alapértelmezett)", foreground="green")
        except Exception:
    ...
    def disable_wu_drivers(self):
        if self.target_os_path and self.target_os_path != self.sys_drive:
            ...
            if not os.path.exists(hive_path):
                ...
                return
            res = subprocess.run(['reg', 'load', 'HKLM\\\\OfflineWUSOFTWARE', hive_path], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            if res.returncode != 0:
                messagebox.showerror("Hiba", f"Reg load hiba: {res.stderr}")
                return
            try:
                p1 = r"HKLM\\\\OfflineWUSOFTWARE\\\\Policies\\\\Microsoft\\\\Windows\\\\WindowsUpdate"
                subprocess.run(['reg', 'add', p1, '/v', 'ExcludeWUDriversInQualityUpdate', '/t', 'REG_DWORD', '/d', '1', '/f'], creationflags=subprocess.CREATE_NO_WINDOW)
                p2 = r"HKLM\\\\OfflineWUSOFTWARE\\\\Microsoft\\\\Windows\\\\CurrentVersion\\\\DriverSearching"
                subprocess.run(['reg', 'add', p2, '/v', 'SearchOrderConfig', '/t', 'REG_DWORD', '/d', '0', '/f'], creationflags=subprocess.CREATE_NO_WINDOW)
                messagebox.showinfo("Siker", f"Windows Update driver telepítés LETILTVA a(z) {self.target_os_path} meghajtón!")
            finally:
                subprocess.run(['reg', 'unload', 'HKLM\\\\OfflineWUSOFTWARE'], creationflags=subprocess.CREATE_NO_WINDOW)
        else:
            try:
    ...
    def enable_wu_drivers(self):
        if self.target_os_path and self.target_os_path != self.sys_drive:
            ...
            try:
                ...
                messagebox.showinfo("Siker", f"Windows Update driver telepítés VISSZAÁLLÍTVA a(z) {self.target_os_path} meghajtón!")
            finally:
                subprocess.run(['reg', 'unload', 'HKLM\\\\OfflineWUSOFTWARE'], creationflags=subprocess.CREATE_NO_WINDOW)
        else:
            try:
...
import re
# Regex to find from __init__ down to just before get_offline_drivers
pattern1 = re.compile(r'    def __init__\(self\):.*?    def get_offline_drivers\(self, all_drivers=False\):', re.DOTALL)
# Regex to find check_wu_status down to just before create_restore_point
pattern2 = re.compile(r'    def check_wu_status\(self\):.*?    def create_restore_point\(self\):', re.DOTALL)

text = pattern1.sub(part1 + '\n    def get_offline_drivers(self, all_drivers=False):', text)
text = pattern2.sub(part2 + '\n    def create_restore_point(self):', text)

# Eliminate old top frame calls from original __init__ since they are obsolete
